...
class GitHubRepositoryManager:
    ...
    def __init__(self, cache_dir: Optional[str] = None) -> None:
        ...
        """
        self.cache_dir = Path(cache_dir or get_settings().repo_cache_dir).resolve() / "github"
    ...
    def clone_or_update(
        ...
        Args:
            ...
            repo_name: Repository full name (owner/repo, used for cache directory)
    ...
    def _clone_repository(
        ...
        """Clone a new repository using secure token authentication."""
        env = os.environ.copy()
        env["GIT_TERMINAL_PROMPT"] = "0"

        parsed_url = urlparse(repo_url)
        if parsed_url.scheme == "https":
            # GitHub uses x-access-token for PAT authentication in clone URLs
            github_token = get_settings().github_token or get_infisical_secret("GITHUB_TOKEN")
