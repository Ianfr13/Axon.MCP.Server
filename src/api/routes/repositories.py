...
@router.get("/repositories/discover/github/{organization}", response_model=GitHubDiscoveryResponse)
async def discover_github_repositories(
    organization: str,
    session: AsyncSession = Depends(get_db_session),
) -> GitHubDiscoveryResponse:
    """
    Discover all repositories in a GitHub organization and check which are already tracked.

    Args:
        organization: GitHub organization name (e.g., "octocat" or "my-org")
    ...
    service = RepositoryService(session)
    try:
        response = await service.discover_github_repositories(organization)
        return response
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
